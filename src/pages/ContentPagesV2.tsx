...
const AD_CONTENT_ROUTES = new Set(['/2', '/2/about', '/2/safety', '/2/how-it-works']);
const INDEXABLE_ROUTES = new Set(['/2', '/2/about', '/2/safety', '/2/how-it-works']);
...
export function SeoGuard() {
  ...
  useEffect(() => {
    ...
    const meta = pageMeta[pathname] ?? pageMeta['/2'];
...
function ContentShell({ children }: { children: ReactNode }) {
  const { pathname } = useLocation();
  const showMobileAppCta = pathname === '/2';

  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <Link to="/2" className="flex items-center gap-3">
          ...
          <nav className="flex items-center gap-3 text-sm text-muted-foreground sm:gap-5">
            <Link to="/2/about" className="hidden hover:text-foreground sm:inline">About</Link>
            <Link to="/2/safety" className="hidden hover:text-foreground sm:inline">Safety</Link>
            <Link to="/2/how-it-works" className="hidden hover:text-foreground sm:inline">How it works</Link>
      ...
      <footer className="border-t border-border/60 px-4 py-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-4">
            <Link to="/2/about" className="hover:text-foreground">About</Link>
            <Link to="/2/safety" className="hover:text-foreground">Safety</Link>
            <Link to="/2/how-it-works" className="hover:text-foreground">How it works</Link>
